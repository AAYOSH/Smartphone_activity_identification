...
if __name__ == '__main__':
    ...
    #==================================================================================
    # using k-fold as cross-validation method 
    k_nb = np.arange(1,21)
    kf = KFold(5)
    test_f1_metric = []
    for n_vizinho in k_nb:
        f1_v = []
        for k,(train_index, test_index) in enumerate(kf.split(X_train,y_train)):
            print('k_fold: {0}'.format(k))
            neigh = KNeighborsClassifier(n_neighbors= n_vizinho)# < mudar metrica
            neigh.fit(X_train.iloc[train_index], y_train.iloc[train_index][0])
            y_knn_predict = neigh.predict(X_train.iloc[test_index])
            _,_,f1 = compute_macro_avg(y_train.iloc[test_index][0],y_knn_predict)
            f1_v.append(f1)
            print(f1)
        test_f1_metric.append(sum(f1_v)/5)


    ##===================================================================================
    # the best f1-metric value obtained in the last step was for k = 16, also shown in image
    neigh = KNeighborsClassifier(n_neighbors= 16)# < mudar metrica
    neigh.fit(X_train, y_train[0])
    y_knn_predict = neigh.predict(X_test)
    _,_,f1 = compute_macro_avg(y_test,y_knn_predict)